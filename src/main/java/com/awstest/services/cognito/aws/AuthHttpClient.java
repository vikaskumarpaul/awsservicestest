...
package com.awstest.services.cognito.aws;
...
final class AuthHttpClient {
	...
	String httpPost(final URL uri, final Map<String, String> bodyParams) throws Exception {
		...
		try {
			...
			for (Map.Entry<String, String> param : bodyParams.entrySet()) {
				reqBuilder.append(URLEncoder.encode(param.getKey(), "UTF-8")).append('=')
	...
	private Map<String, String> getHttpHeader() {
		...
		httpHeaderParams.put(Constants.HTTP_HEADER_PROP_CONTENT_TYPE, Constants.HTTP_HEADER_PROP_CONTENT_TYPE_DEFAULT);
